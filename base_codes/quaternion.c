...
// quaternion, vector and matrix operations
...
tensor getrotmatrix(quaternion q) {
 // returns the roation matrix based on quaternion q
    tensor mat;
...
quaternion normalize_quat(quaternion a){
    // it normalizes the quaternion to unit length
    quaternion b;

    scdivide_quat(a,sqrt(quat_inp(a,a)),b);
...
quaternion QuaternionXaxis( double degree) {
    // rotates the particles around the x-axis
    quaternion q;
    double alpha,sina;
...
// rotates the particles around the y-axis for degree degrees 
    quaternion q;
    double alpha,sina;
...
// rotates the particles around the z-axis
    quaternion q;
    double alpha,sina;
