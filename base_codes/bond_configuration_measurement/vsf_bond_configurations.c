...
void set_reduced_bondnumber(Slice *)
/*---------------------------------------------------------------------------*/
...
void version_specific_analysis(Slice *psl){
    ...
    if (start_slice->nbonds!=psl->nbonds){
        // restart from a new configuration. 
        restart_conf(psl);
    }
...
/* *********** INITIALIZE STUFF**************/
   // all for measureing the P(E), P(S), and P(r_ss); do this after total_energy() has been performed
 
 void initialize_bond_configurations_measurements(Slice *psl){
    set_reduced_bondnumber(psl);
...
void make_histogram(Histogram *hist, double min, double max, double d_bin){
   ...
   if (hist->nbins>MAXBIN) error("nbins>MAXBIN");

   return;
}

void    set_reduced_bondnumber(Slice *psl){
        ...
        for (int L=0; L<Lchain; L++){
            ...
            for (int n=0; n<psl->pts[DPc].nbonds;n++){
                ...
                if ((checkElementXIntArray(&DP_list, bound_particle )==1) || (bound_particle<DP1) ){
                    ...
                    red_bonds.pts_pair[redbond_nr].i[red_bonds.pts_pair[redbond_nr].nbonds]=DPc;
                    red_bonds.pts_pair[redbond_nr].j[red_bonds.pts_pair[redbond_nr].nbonds]=bound_particle;
                    red_bonds.pts_pair[redbond_nr].nbonds+=1;
