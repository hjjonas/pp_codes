...
void removeElementXIntArray(IntArray *a, int elemX ){
  ...
  if(a->ints[last_int]==elemX){
    ...
    for (i=0;i<a->used;i++){
      if(a->ints[i]==elemX){
        start=1;
        printf("    removeElementXIntArray at a->ints[%d] == %d\n",i,elemX);
...
//    #####  #####  #####  #####  ##### 2D IntsArray  ##### #####  #####  #####  ##### 
void initIntsArray(IntsArray *a, size_t initialSize ) {
  //https://stackoverflow.com/questions/3536153/c-dynamically-growing-array
  //size_t initialSize is the intial size of the array. for example "5" intergers, do not keep it too small, as the array is doubled if it is too short 
  // a->used is the number of used entries, upon initialization it is zero.

  if(((a->ipart = (int *) malloc(initialSize * sizeof(int) ))==NULL)) {;
     error("could not malloc initialSize * sizeof(int) ");
  }
  
  if(((a->isite = (int *) malloc(initialSize * sizeof(int) ))==NULL)) {;
     error("could not malloc initialSize * sizeof(int) ");
  }
...
void insertIntsArray(IntsArray *a, int element_ipart, int element_isite) {
  // the original array a is elongated if you want to add more elements that is currently has. Just for easiness it is doubled. 
  // NOT handy when the array becomes big!
  // a->used is the number of used entries, because a->ints[a->used++] updates a->used only *after* the array has been accessed.
  ...
  if (a->used == a->size-1) {
    a->size *= 2;
    if( (a->ipart = realloc(a->ipart, a->size * sizeof(int)))==NULL) {;
       error("could not realloc IntsArray->ints ");
    }
    if( (a->isite = realloc(a->isite, a->size * sizeof(int)))==NULL) {;
       error("could not realloc IntsArray->ints ");
    }
    // a->ints = realloc(a->ints, a->size * sizeof(int)); // reallocate the elongated array with the new size.
  }
  a->ipart[a->used] = element_ipart;
  a->isite[a->used] = element_isite;
  a->used++;
  return;

}

void freeIntsArray(IntsArray *a) {
  free(a->ipart); //free the memory
  free(a->isite); //free the memory
  a->ipart = NULL;// make pointer zero
  a->isite = NULL;// make pointer zero
...
void removeIthElementIntsArray(IntsArray *a, int elemi ){
  // elemi stands for the ith element 
  // the array a has a->used  elements currently
  int start =0,i;
  ...
  a->used--;
  for (i=elemi;i<a->used;i++){
      a->ipart[i]=a->ipart[i+1];
      a->isite[i]=a->isite[i+1];
  }
  return;
}

void removeElementXIntsArray(IntsArray *a, int elem_ipart, int elem_isite ){
  // elemX stands for element number X
  // the array a has a->used  elements currently
  int start =0,i;
  int last_int=(int)a->used-1;
  ...
  if((a->ipart[last_int]==elem_ipart) && (a->isite[last_int]==elem_isite)){
      a->used--;
      return;
  }
  else {
    a->used--;
    for (i=0;i<a->used;i++){
      if((a->ipart[i]==elem_ipart) && (a->isite[i]==elem_isite)){
        start=1;
        // printf("    removeElementXIntsArray at a->ipart[%d] == %d == %d \n",i,a->ipart[i],elem_ipart);
        // printf("    removeElementXIntsArray at a->isite[%d] == %d  == %d \n",i,a->isite[i],elem_isite);
  ...
  if (start==0   ){
    // dprint((int)a->used+1);
    dprint(a->ipart[last_int]);
    printf("you want to remove elemnt ipart=%d isite=%d\n",elem_ipart,elem_isite);
    printf("the remaining elements are :\n");
...
int checkElementXIntsArray(IntsArray *a, int elem_ipart, int elem_isite){
  // check if elementX is in the array
  // the array a has a->used  elements currently
  int check =0,i;
...
void printIntsArray(IntsArray *a ){
  // check if elementX is in the array
  // the array a has a->used  elements currently
  int i;

  printf(" >> length of IntsArray = %zu\n", a->used);
  for (i=0;i<a->used;i++){
    printf(" item %d has value ipart=%d  isite=%d\n", i,a->ipart[i],a->isite[i]);
...
void printBondTimeInfoArray(BondTimeInfoArray *a ){
  ...
  for (i=0;i<a->used;i++){
    printf(" bond %d connected at %3.2lf has connection between ipart %d (isite=%d) and jpart %d(jsite=%d) \n", 
      i,a->bondinfos[i].start_time,
      a->bondinfos[i].ipart,a->bondinfos[i].isite,
      a->bondinfos[i].jpart,a->bondinfos[i].jsite);
