...
typedef struct {
    const char *name;
    void *ptr;
    char type; // 'd' = int, 'f' = float (double), 's' = string, 'p' = print section, 'n' = newline
} Entry;
...
void read_input(Slice *psl) {
    ...
    Entry entries[] = {
        ...
        {"start_type", &sys.start_type, 'd'},
        {"read_path", &sys.read_path, 'd'},
        {"directorypath", sys.directorypath, 's'},
        {"mc_warmup", &sys.mc_warmup, 'd'},
        {"particle_setup", &sys.particle_setup, 'd'},
        {"ncycle1", &sys.ncycle1, 'd'},
        {"ncycle2", &sys.ncycle2, 'd'},
        {"graphics", &sys.graphics, 'd'},
        {"cluster_MC", &sys.cluster_MC, 'd'},
        {"nearest_neighbor", &sys.nearest_neighbor, 'd'},
        {"restart", &sys.restart, 'd'},
        {"empty_files", &sys.empty_files, 'd'},
        ...
        {"boxl", &sys.boxl.x, 'f'},
        {"boxly", &sys.boxl.y, 'f'},
        {"nchains", &sys.nchains, 'd'},
        {"chaingap", &sys.chaingap, 'f'},
        // ----- analysis -----
        {"bond_breakage", &analysis.bond_breakage, 'f'},
        ...
        {"cluster_analysis", &psl->cluster.analysis, 'd'},
        ...
        for (int i = 0; i < n_entries; i++) {
            if (strcmp(pt, entries[i].name) == 0) {
                ...
                switch (entries[i].type) {
                    case 'd':
                        pt = strtok(NULL, " ");
                        if (pt) sscanf(pt, "%d", (int *)entries[i].ptr);
                        break;
                    case 'f':
                        pt = strtok(NULL, " ");
                        if (pt) sscanf(pt, "%lf", (double *)entries[i].ptr);
                        break;
                    case 's':
                        pt = strtok(NULL, " ");
                        if (pt) sscanf(pt, "%s", (char *)entries[i].ptr);
                        break;
                    case 'p':
...
void init_model(Slice *psl) {
    ...
    */
    ...
    pot.bond_cutoffE=0; // this defines the bond
...
void init_simtype(Slice *psl){
    ...
    if(psl->cluster.analysis==1){
        ...
        strcpy(slice[0].cluster.size_histogram.filename,"clustersize_histogram.out");
        strcpy(slice[0].cluster.size_distribution.filename,"clustersize_distribution.out");

        if(sys.restart){
            read_statistics_file(&slice[0].cluster.size_histogram);
            read_statistics_file(&slice[0].cluster.size_distribution);          
        }   
    }
