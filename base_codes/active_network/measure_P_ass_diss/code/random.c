...
double RandomGaussianNumber() {
  //Hannah: I found this code on stackoverflow that is similar to this one
...
vector RandomVector1(void) {
    // random point in box x,y,z in  [-1,1]
   vector u;

    u.x=RandomNumberRange(-1., 1.);
    u.y=RandomNumberRange(-1., 1.);
    u.z=RandomNumberRange(-1., 1.);

    return u;
}

vector RandomVector(double drmax) {
    // random point in box x,y,z in  [-drmax,drmax]
    vector u;
...
vector RandomVector3D(vector drmax) {
    //   // random point in box x,y,z box drmax
    vector u;

    u=RandomVector1();

    vector_times(drmax,u,u);
